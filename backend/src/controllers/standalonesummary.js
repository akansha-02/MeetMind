...
export const uploadAndSummarize = async (req, res, next) => {
  try {
    if (!req.file) {
      return res
        .status(400)
        .json({ message: "No audio or video file provided" });
    }
    ...
    if (!transcriptionResult.transcript) {
      return res
        .status(400)
        .json({ message: "Transcription failed or produced no content" });
    }
    ...
    res.json({
      ...
      message: `Summary generated successfully using ${provider.toUpperCase()}`,
    });
  } catch (error) {
    next(error);
  }
